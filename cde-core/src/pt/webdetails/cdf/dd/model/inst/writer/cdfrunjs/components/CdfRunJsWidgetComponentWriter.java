 ...
 * @author dcleao
 */
public class CdfRunJsWidgetComponentWriter extends JsWriterAbstract implements
		IThingWriter {
	private static final Log _logger = LogFactory
			.getLog(CdfRunJsWidgetComponentWriter.class);

	@Override
	public void write(Object output, IThingWriteContext context, Thing t)
			throws ThingWriteException {
		this.write((StringBuilder) output,
				(CdfRunJsDashboardWriteContext) context, (WidgetComponent) t);
	}

	public void write(StringBuilder out, CdfRunJsDashboardWriteContext context,
			WidgetComponent comp) throws ThingWriteException {
		...
		CdfRunJsDashboardWriteResult dashResult = dashMgr.getDashboardCdfRunJs(
				comp.getWcdfPath(), options, context.isBypassCacheRead());
	...
	private void writeParameters(StringBuilder out, WidgetComponent comp,
			String aliasPrefix) {
		if (comp.getPropertyBindingCount() > 0) {
			...
			for (PropertyBinding prop : props) {
				if ("parameters".equalsIgnoreCase(prop.getAlias())
						|| "xActionArrayParameter".equalsIgnoreCase(prop
								.getName())) // legacy way
				{
					String paramsAssocList = prop.getValue();
					if (StringUtils.isNotEmpty(paramsAssocList)) {
						this.writeParametersAssocList(out, paramsAssocList,
								aliasPrefix);
					}
					...
					writeJsSyncParameter(out, prop.getAlias(), prop.getValue(),
							aliasPrefix);
				}
			}
		}
	...
	private void writeParametersAssocList(StringBuilder out,
			String paramsAssocList, String aliasPrefix) {
		try {
			...
			for (int i = 0, L = params.length(); i < L; i++) {
				...
				writeJsSyncParameter(out, line.getString(0), dashParam,
						aliasPrefix);
			}
		} catch (JSONException ex) {
			_logger.error(
					"Could not write widget parameters in association list", ex);
		}
	}

	private void writeJsSyncParameter(StringBuilder out,
			String widgetLocalProp, String dashParam, String aliasPrefix) {
